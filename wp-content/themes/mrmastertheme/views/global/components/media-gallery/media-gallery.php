<?php
    ...
    if ($media_gallery) :
        ...
        echo get_template_part('views/global/components/media-gallery/components/title-area', null, array(
        ...
        if ($layout === 'carousel') {
            echo get_template_part('views/global/components/media-gallery/layout-options/carousel', null, array('media_gallery' => $media_gallery));
        } elseif ($layout === 'masonry') {
            echo get_template_part('views/global/components/media-gallery/layout-options/masonry', null, array('media_gallery' => $media_gallery));
        }
?> 
...
<?php
        ...
        foreach ($media_gallery as $position => $slide) {
            if ($slide['media_type']) {
                $item_list['itemListElement'][] = [
                    ...
                    'item'     => [
                        '@type'      => 'ImageObject',
                        'contentUrl' => $slide['slide_image']['url'],
                    ],
                ];
            } else {
                $video_item = [
                    '@type' => 'VideoObject',
                    'url'   => $slide['slide_video']['video_link'],
                ];
                if (!empty($slide['slide_video']['poster_image'])) {
                    $video_item['thumbnailUrl'] = $slide['slide_video']['poster_image']['url'];
                }
                if (!empty($slide['slide_video']['video_description'])) {
                    $video_item['description'] = wp_strip_all_tags($slide['slide_video']['video_description']);
                }
                $item_list['itemListElement'][] = [
                    '@type'    => 'ListItem',
                    'position' => $position + 1,
        ...
        echo '<script type="application/ld+json">' . json_encode($item_list, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) . '</script>';
        echo $closing_tag;
    endif;
?>
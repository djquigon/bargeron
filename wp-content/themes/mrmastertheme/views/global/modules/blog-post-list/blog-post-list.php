...
if ($module_id && $module_class_names) {
    $opening_tag = '<' . $tag_type . ' id="' . $module_id . '" class="blog-post-list ' . $module_class_names . '" data-layout="' . $layout . '">';
} else if ($module_id && !$module_class_names) {
    $opening_tag = '<' . $tag_type . ' id="' . $module_id . '" class="blog-post-list" data-layout="' . $layout . '">';
} else if (!$module_id && $module_class_names) {
    $opening_tag = '<' . $tag_type . ' class="blog-post-list ' . $module_class_names . '" data-layout="' . $layout . '">';
} else {
    $opening_tag = '<' . $tag_type . ' class="blog-post-list" data-layout="' . $layout . '">';
}
...
//declare variable to hold articles:
$articles = get_sub_field('blogs');

//if no articles are selected from the module's sub-field
if (!$articles) {
    //exclude the 'Featured Article' if it is set from the Blog Archive Page's ACF:
    if (get_field('featured_post', get_option('page_for_posts'))) {
        $excluded_featured_post = get_field('featured_post', get_option('page_for_posts'));

        $articles = get_posts(array(
            'post_type' => 'post',
            'posts_per_page' => 3,
            'post__not_in' => [$excluded_featured_post->ID]
        ));
    } else {
        $articles = get_posts(array(
            'post_type' => 'post',
    ...
    $no_posts_provided = true; 
} else {
    $no_posts_provided = false;
}

//declare variable to hold 'All' button:
$all_blogs_button = get_sub_field('all_blogs_button');

//decide whether the content should be overlayed over the featured image or if it should be separate
$overlay_content = get_sub_field('overlay_content');

?>

<?php
//we're only generating HTML if the module has blogs to print
if ($articles) :
    echo $opening_tag;
    ...
    if ($layout === 'cards') {
        ...
        $template_args = array(
            ...
            'all_blogs_button' => $all_blogs_button,
            'no_posts_provided' => $no_posts_provided
        );

        echo get_template_part('views/global/modules/blog-post-list/layout-options/cards', null, $template_args);
        ...
        echo get_template_part('views/global/modules/blog-post-list/layout-options/featured-article', null, $template_args);
    }
?>

...
//get the selected layout option from settings (slider, cards):
$layout = get_sub_field('layout');

//grab the column count from settings (applicable to both layout options):
...
if ($module_id && $module_class_names) {
    $opening_tag = '<' . $tag_type . ' id="' . $module_id . '" class="team-members ' . $module_class_names . '" data-layout="' . $layout . '">';
} else if ($module_id && !$module_class_names) {
    $opening_tag = '<' . $tag_type . ' id="' . $module_id . '" class="team-members" data-layout="' . $layout . '">';
} else if (!$module_id && $module_class_names) {
    $opening_tag = '<' . $tag_type . ' class="team-members ' . $module_class_names . '" data-layout="' . $layout . '">';
} else {
    $opening_tag = '<' . $tag_type . ' class="team-members" data-layout="' . $layout . '">';
}
...
//declare variable to hold team members:
$team_members = get_sub_field('team_members');

//options for how to display the bio:
$bio_options = get_sub_field('bio_options');

$all_team_members_button = get_sub_field('all_team_members_button');
...
$template_args = array(
    ...
    'all_team_members_button' => $all_team_members_button,
...
<?php
//we're only generating HTML if the module has team members to print
if ($team_members) :
    echo $opening_tag;

    //depending on the chosen layout option, we grab the appropriate template file:
    if ($layout === 'slider') {
        echo get_template_part('views/global/modules/team-members/layout-options/slider', null, $template_args);
    } else if ($layout === 'cards') {
        echo get_template_part('views/global/modules/team-members/layout-options/cards', null, $template_args);
    }
?>

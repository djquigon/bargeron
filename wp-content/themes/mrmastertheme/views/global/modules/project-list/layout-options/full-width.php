<?php
    ...
    if ($project_list) :
?>
        <div id="project-carousel-<?= $random_integer; ?>" class="project-list-row container">
            <?php
                foreach ($project_list as $list_item) :
                    $project_post_object = $list_item;

                    $project_id = $project_post_object->ID;
                ...
                if (get_post_thumbnail_id($project_id)) {
                    ...
                    if ($project_media_gallery[0]['media_type']) {
                        ...
                        if ($project_media_gallery[0]['slide_video']['poster_image']) {
                            ...
                            // for this particular module layout, because of the hover state styling, we actually don't care about generating a clickable video link. We're just directing the user to the project's single view
                        }                       
                    }
                } else {
            ...
            ?>
                    <div class="project-slide" <?= $text_color_attribute ?>>
                        <?php
                            //if we have an image associated with the Project, we use it:

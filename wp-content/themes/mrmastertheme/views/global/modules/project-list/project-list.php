...
if ($module_id && $module_class_names) {
    $opening_tag = '<' . $tag_type . ' id="' . $module_id . '" class="project-list ' . $module_class_names . '" data-layout="' . $layout . '">';
} else if ($module_id && !$module_class_names) {
    $opening_tag = '<' . $tag_type . ' id="' . $module_id . '" class="project-list" data-layout="' . $layout . '">';
} else if (!$module_id && $module_class_names) {
    $opening_tag = '<' . $tag_type . ' class="project-list ' . $module_class_names . '" data-layout="' . $layout . '">';
} else {
    $opening_tag = '<' . $tag_type . ' class="project-list" data-layout="' . $layout . '">';
}
...
//declare variable to hold the list of project CPT posts if 'Manual Selection' was selected
if ($manual_selection_or_automatic) {
    $project_list = get_sub_field('project_list');
} else {
    // automatically pulls in the 6 most recent published project CPT posts
    $query_args = [
        'post_type' => 'mandr_project',
        'status' => 'publish',
        'posts_per_page' => 6
    ];
    $project_list = get_posts($query_args);
}

//declare variable to hold the list of project CPT posts if 'Manual Selection' was selected for the featured layout
if ($featured_manual_selection_or_automatic) {
    $featured_project = get_sub_field('featured_project');
} else {
    // automatically pulls in the most recently published project CPT post
    $query_args = [
        'post_type' => 'mandr_project',
        'status' => 'publish',
        'posts_per_page' => 1
    ];
    $featured_project = get_posts($query_args);
}
...
$template_args = array(
    ...
    'all_projects_button_text' => $all_projects_button_text,
    'all_projects_button_link' => $all_projects_button_link,
...
if ($project_list) :
    ...
    echo get_template_part('views/global/modules/project-list/components/title-area', null, $template_args);

    //depending on the chosen layout option, we grab the appropriate template file:
    if ($layout === 'carousel') {
        echo get_template_part('views/global/modules/project-list/layout-options/carousel', null, $template_args);
    } else if ($layout === 'featured') { 
        // For THIS layout, the $template_args will need to be adjusted above
        echo get_template_part('views/global/modules/project-list/layout-options/featured', null, $template_args);
    } else if ($layout === 'full-width') {        
        echo get_template_part('views/global/modules/project-list/layout-options/full-width', null, $template_args);
    }
?>
...
<?php
    if ($layout !== 'featured') :
        ...
        foreach ($project_list as $position => $project) {
        ...
        echo '<script type="application/ld+json">' . json_encode($item_list_schema, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) . '</script>';
    endif;

...
function mandr_custom_post_type_initialization()
{
    ...
    //Testimonials
    if (get_field('mandr_cpt_testimonials', 'options')) {
...
//An option to hide the default 'Posts' tab. This is for sites that don't need it because they don't have a blog:
function mandr_hide_posts_menu_item()
{
    if (!get_field('mandr_posts', 'options')) {
        remove_menu_page('edit.php');
    }
}
add_action('admin_menu', 'mandr_hide_posts_menu_item');


function mandr_remove_default_post_type_menu_bar( $wp_admin_bar ) {
    if (!get_field('mandr_posts', 'options')) {
        $wp_admin_bar->remove_node( 'new-post' );
    }
}
add_action( 'admin_bar_menu', 'mandr_remove_default_post_type_menu_bar', 999 );